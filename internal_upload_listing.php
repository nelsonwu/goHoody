<?php  
	...
	if (@$_POST['newbutton'] == "Cancel")
		header("Location: http://gohoody.com/dashboard/");
		
	// grab data from create_listing.php
	$user = $_POST['seller_uid'];
	$title = $_POST['title'];
...
{	
	...
	foreach($active_keys as $key)
	{
		...
		$result = mysql_query($query) or die (fatal_error(286, $user, $user, $today, $query, mysql_error()));
	...
	if ($lid > 0) 
	{
		...
		$result = mysql_query($query) or die (fatal_error(287, $user, $user, $today, $query, mysql_error()));
		...
		$result = mysql_query($query) or die (fatal_error(288, $user, $user, $today, $query, mysql_error()));
		...
		if ($location1 == "at_home")
		{
			...
			$result = mysql_query($query) or die (fatal_error(289, $user, $user, $today, $query, mysql_error()));
		...
		else if ($location1 == "away_home")
		{
			if ($location2 == "buyer_home")
			{
				...
				$result = mysql_query($query) or die (fatal_error(290, $user, $user, $today, $query, mysql_error()));
			}
			else if ($location2 == "other")
			{
				$sql = "SELECT country,state FROM User_Address WHERE fb_uid='$user'";
				$result = mysql_query($sql) or die (fatal_error(291, $user, $user, $today, $sql, mysql_error()));
				...
				$result = mysql_query($query) or die (fatal_error(292, $user, $user, $today, $query, mysql_error()));

			}
		} // End of if ($location1 = "away_home")
		...
		if ($keep_pictures == 0 && $picture_count != 0)
		{
			...
			$result = mysql_query($query) or die (fatal_error(293, $user, $user, $today, $query, mysql_error()));
		}	
		else if ($picture_count != 0)
		{
			...
			while ($counter2 <= 5)
			{
				...
				else
				{
					...
					$result = mysql_query($query) or die (fatal_error(294, $user, $user, $today, $query, mysql_error()));
					$counter1++;
					$counter2++;
				}	
			}
			...
			$result = mysql_query($query) or die (fatal_error(295, $user, $user, $today, $query, mysql_error()));
		}	
	} // End of if ($lid > 0)
...
{	
	...
	if(empty($upload_pictures))
	{
		...
		foreach($active_keys as $key)
		{
			...
			//Insert data into the Listing_Pictures table
			$query = "INSERT INTO Pictures_Lookup (URL) VALUES('$upload_filename')";
			$result = mysql_query($query) or die (fatal_error(296, $user, $user, $today, $query, mysql_error()));
			$picture_index[] = mysql_insert_id();	
		}
	...
	$result = mysql_query($query) or die (fatal_error(297, $user, $user, $today, $query, mysql_error()));
	...
	if(empty($upload_pictures))
	{
		if ($picture_count == 0)
		{
			...
			$result = mysql_query($query) or die (fatal_error(302, $user, $user, $today, $query, mysql_error()));
		}
		else
		{
			// Insert data into the Listing_Pictures table
			...
			$result = mysql_query($query) or die (fatal_error(303, $user, $user, $today, $query, mysql_error()));
		}
	}
	...
	else
	{
		...
		$result = mysql_query($query) or die (fatal_error(304, $user, $user, $today, $query, mysql_error()));
